...
error rates:
comedy highest
false negative very small - drama largest
false positive: drama & documentary near average - only comedy much higher
large improvements at the beginning
zick-zack curves again - for drama and comedy

Cause:
better quality data/fewer movies with all features?
Combination is actually better cause more diverse and precisce
...
TODO:
drama, comedy & documentary always highest error - drama most false-negative, comedy most false positive -> 3 largest genres -> fuzziest/diffuse genres
...
future work: more fetaures, try different feature combinations, filter for only relevant/high quality movies/data, investigate zick-zack curves, investigate if better classification is actually cause of better method or just better data (run single feature versions only on the subset of data with all 3 features)
Spark provides nice interface and ML lib -> easy to use
preproccessing neccessary for spark (data format)
