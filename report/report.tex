...
\par Scala: Scala is a programming language for general software applications. It has a built-in type inference mechanism which allows the programmer to omit certain type annotations. For instance, it is not necessary to specify the type of a variable, since the compiler can deduce the type from the initialization expression of the variable. The conciseness of Scala has a productivity benefit in allowing developers to do more functional work in a given workday. Additionaly, it is also a full-blown functional language. Except for the first-class functions, a library with efficient immutable data structures and a general preference of immutability over mutation are included in Scala. Most Scala data frameworks have the notion of some abstract data type that's extremely consistent with Scala's collection API. Similarly, RDD in Spark also have the same set of methods, e.g. map, flatMap, filter, reduce, fold and groupBy. One could just learn the standard collection and easily pick up one of the libraries. 
\section{Methodology}
...
\end{itemize}
The real data is between the head line and the seperator line and still not good formatted. For example, in plot.list, plot description is not a single line as we expected for the input of Spark pipeline, but split into several lines and begin with the notation "PL". So we need to scan and preprocess the raw data with small Python scripts and simple Scala programs. 
